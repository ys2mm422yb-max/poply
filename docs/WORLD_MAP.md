...
This first H slice is a world/progression viewer and safe revisit surface. It does not change which chapter owns live orders or generators. That gameplay-selection rule stays untouched while the independent replacement-order balancing PR owns `src/v2-game.js`.

A later H slice may add explicit active-Place switching only after its order/generator implications are defined and tested without losing progress.
