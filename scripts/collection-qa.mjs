...
try{
  ...
  const before=await readSave();assert(before.discoveries.includes('item:coffee:1'),'starting coffee tier 1 not backfilled');assert(!before.discoveries.includes('item:coffee:2'),'future coffee tier leaked into collection');assert(!before.discoveries.includes('item:herb:1'),'future Dachgarten tier leaked into collection');
  ...
  await page.waitForFunction(()=>{const el=document.querySelector('.discovery-reveal');if(!el)return false;const opacity=Number(getComputedStyle(el).opacity);return el.classList.contains('is-visible')&&!el.classList.contains('is-leaving')&&opacity>=.95;},null,{timeout:1600});
  ...
  assert((await page.locator('.collection-total strong').textContent())==='4/30','collection total does not include the new tier and Dachgarten capacity');
  assert(await page.locator('.collection-family').count()===5,'Collection must expose five item families');
  ...
  await page.locator('[data-collection-family="herb"]').click();assert((await page.locator('.collection-focus h2').textContent())?.includes('Dachgarten'),'Dachgarten family did not open');assert(await page.locator('.collection-tier.locked').count()===6,'undiscovered Dachgarten tiers should all remain silhouettes');await shot('33-collection-locked-garden');

  await page.setViewportSize({width:390,height:720});await page.waitForTimeout(120);await assertFits('390x720 collection');await shot('34-collection-short-safari');
  ...
  report={discoveries:reloaded.discoveries,playerXp:reloaded.playerXp,revealVisual,families:5,totalItems:30};
