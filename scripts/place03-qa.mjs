...
try{
  ...
  await page.locator('.nav-tab[data-view="place"]').click();await page.waitForSelector('.view-place.place-garden');assert((await page.locator('.world-copy h1').textContent())==='Dachgarten','Dachgarten Place did not render');assert(await page.locator('.garden-place-svg').count()===1,'authored Dachgarten scene missing');assert(await page.locator('.garden-construction-base').count()===1,'stage zero construction base missing');assert(await page.locator('.garden-greenhouse-built').count()===0,'Gewächshaus must not already exist at stage zero');assert((await page.locator('.place-current-goal').textContent())?.includes('Glasdach'),'first Dachgarten restoration goal missing');await shot('83-place03-place-stage0');
  const beforeBuild=await readSave();await page.locator('[data-action="build"]').click();await page.waitForFunction(()=>{try{return JSON.parse(localStorage.getItem('poply-v2-state-1')||'null')?.placeUpgrades?.includes('garden-glass');}catch{return false;}},null,{timeout:1500});const afterBuild=await readSave();assert(afterBuild.stars===beforeBuild.stars-12,`Glasdach should cost 12 stars, got ${beforeBuild.stars} -> ${afterBuild.stars}`);assert((await page.locator('.world-progress b').textContent())==='1','Dachgarten stage did not advance to 1');assert(await page.locator('.garden-greenhouse-built').count()===1,'Glasdach build did not visibly create the greenhouse');await page.waitForTimeout(1850);assert(await page.locator('.restoration-reveal').count()===0,'restoration reveal did not clear before stable screenshot');await shot('84-place03-place-stage1');
