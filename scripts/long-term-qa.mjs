...
const compactMasteryState=()=>page.evaluate(()=>['coffee-gen','pantry-gen'].map(id=>{const card=document.querySelector(`[data-generator-mastery="${id}"]`),small=card?.querySelector('small'),strong=card?.querySelector('strong'),em=card?.querySelector('em'),cardRect=card?.getBoundingClientRect(),strongRect=strong?.getBoundingClientRect(),emRect=em?.getBoundingClientRect();const visible=node=>node&&getComputedStyle(node).display!=='none'&&node.getBoundingClientRect().width>0&&node.getBoundingClientRect().height>0;const inside=rect=>cardRect&&rect&&rect.top>=cardRect.top-1&&rect.bottom<=cardRect.bottom+1;return {id,smallDisplay:small?getComputedStyle(small).display:null,strongVisible:visible(strong),emVisible:visible(em),strongText:strong?.textContent||'',emText:em?.textContent||'',strongInside:inside(strongRect),emInside:inside(emRect)};}));

let report={},failure=null,shortMasteryCompact=false;
try{
  ...
  for(const height of [844,720]){
    ...
    if(height<=760){const compact=await compactMasteryState();const coffeeCompact=compact.find(entry=>entry.id==='coffee-gen'),pantryCompact=compact.find(entry=>entry.id==='pantry-gen');assert(compact.every(entry=>entry.smallDisplay==='none'&&entry.strongVisible&&entry.emVisible&&entry.strongInside&&entry.emInside),`compact generator mastery clips or duplicates status ${JSON.stringify(compact)}`);assert(coffeeCompact?.strongText==='Kaffeemaschine'&&coffeeCompact?.emText==='MEISTER',`compact coffee mastery hierarchy wrong ${JSON.stringify(coffeeCompact)}`);assert(pantryCompact?.strongText==='Vorratskiste'&&pantryCompact?.emText==='24/50',`compact pantry mastery hierarchy wrong ${JSON.stringify(pantryCompact)}`);shortMasteryCompact=true;}
  ...
  report={generatorMastery:true,generatorMasteryDomain:true,shortMasteryCompact,dailyStories:true,collectionSilhouettesOnly:true,activeOrdersNoUnknownPlaceholder:true,worldCompletion:true,reducedMotion:true,viewports:['390x844','390x720'],noDocumentScroll:true};
