...
const applySafeArea=async()=>{
  await page.evaluate(safeTop=>{
    document.documentElement.style.setProperty('--poply-safe-top',`${safeTop}px`);
  },SAFE_TOP);
  await page.waitForTimeout(100);
};

async function metrics(label){
  const data=await page.evaluate(safeTop=>{
    ...
    return {
      safeTop,
      safeVar:getComputedStyle(document.documentElement).getPropertyValue('--poply-safe-top').trim(),
      viewportHeight:window.visualViewport?.height||innerHeight,
      innerHeight,
      scrollHeight:document.documentElement.scrollHeight,
      app:rect(document.querySelector('.app-shell')),
      topbar:rect(topbar),brand:rect(brand),badge:rect(badge),actions:rect(actions),menu:rect(menu),resources
    };
  },SAFE_TOP);
  assert(data.safeVar===`${SAFE_TOP}px`,`${label}: safe-area override missing ${JSON.stringify(data)}`);
  assert(data.app&&data.topbar&&data.brand&&data.badge&&data.actions&&data.menu,`${label}: header nodes missing ${JSON.stringify(data)}`);
...
try{
  ...
  await writeFile(`${outDir}/standalone-safe-area-report.json`,JSON.stringify({safeTop:SAFE_TOP,report,problems,failure:failure?.message||null},null,2));
  await browser.close();
}
