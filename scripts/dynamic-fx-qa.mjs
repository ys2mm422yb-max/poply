...
try{
  ...
  await page.reload({waitUntil:'networkidle'});await page.waitForSelector('.view-board');await assertFits('dynamic FX 390x844');
  ...
  const discovery=page.locator('.discovery-reveal');await discovery.waitFor({state:'visible'});await page.waitForTimeout(180);
