...
try{
  ...
  assert(await page.locator('[data-select-order="order-0"] .service-special-choice').count()===0,'Erster Kaffee should stay simple');
  assert(((await page.locator('[data-select-order="order-1"] .service-special-choice').textContent())||'').includes('SERIE'),'second opening guest does not expose Merge-Serie');
  assert(((await page.locator('[data-select-order="order-2"] .service-special-choice').textContent())||'').includes('FRISCH'),'third opening guest does not expose Fresh Special');
  ...
  await page.reload({waitUntil:'networkidle'});await openOrders();await page.locator('[data-select-order="order-1"]').click();
  ...
  await page.setViewportSize({width:390,height:720});await reset();await openOrders();await page.locator('[data-select-order="order-1"]').click();
  ...
  report={openingSpecials:specials,seriesRewardCoins:40,completedWithMerges:2,deliveryCoinsWithBonus:130,replacementSpecial:replacement.special.type,shortViewportNoScroll:true};
