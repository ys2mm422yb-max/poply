...
test('Block 6 wiring keeps long-term motivation inside existing surfaces',async()=>{
  ...
  assert.match(main,/installLongTermUI\(root\)/);assert.match(index,/aaa-long-term\.css\?v=20260821-longterm1/);assert.match(ui,/world-complete-payoff/);assert.doesNotMatch(ui,/saveGameState|localStorage|coins\s*[+\-=]|stars\s*[+\-=]/);assert.match(css,/generator-discovery\.mastered/);assert.match(css,/generator-discovery\.known small\{display:none\}/);assert.match(css,/daily-story-rule/);assert.match(css,/world-complete-payoff/);assert.match(css,/prefers-reduced-motion/);assert.match(workflow,/Run long-term motivation WebKit QA/);
});
