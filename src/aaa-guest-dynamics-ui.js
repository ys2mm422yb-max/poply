...
export function installGuestDynamicsUI(root){
  ...
  const decorate=()=>{
    ...
    root.querySelectorAll('[data-select-order]').forEach(choice=>{
      ...
      choice.title=`${choice.title?`${choice.title} · `:''}${trait.label}: ${trait.copy}`;
    });
    root.querySelectorAll('.service-card[data-service-order]').forEach(card=>{
      ...
      const existing=heading.querySelector('.guest-dynamic-line');existing?.remove();
      const bonus=dynamicServiceBonus(state,order),line=document.createElement('div');line.className='guest-dynamic-line';
      const traitOn=guestTraitQualifies(order,bonus.trait.id);
      const dailyOn=order.requirements.some(req=>req.family===condition.family);
      line.innerHTML=`<span class="guest-trait ${traitOn?'is-earned':''}"><b>${bonus.guest.name}</b> · ${bonus.trait.label}${traitOn?` <strong>+${bonus.trait.bonusCoins} ●</strong>`:''}</span><span class="daily-condition ${dailyOn?'is-earned':''}">Heute: ${condition.label} · +${condition.bonusCoins} ● bei ${familyLabel[condition.family]||condition.family}</span>`;
      heading.append(line);
    });
  };
