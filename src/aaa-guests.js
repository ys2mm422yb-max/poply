export const GUEST_PROFILES=Object.freeze([
  Object.freeze({id:'mika',name:'Mika'}),
  Object.freeze({id:'nora',name:'Nora'}),
  Object.freeze({id:'sam',name:'Sam'})
]);

export const GUEST_LOYALTY_MILESTONES=Object.freeze([
  Object.freeze({visits:1,title:'Bekannt',rewardCoins:25}),
  Object.freeze({visits:5,title:'Stammgast',rewardCoins:100}),
  Object.freeze({visits:12,title:'Lieblingsgast',rewardCoins:250})
]);
...
export function ensureGuestState(inputState){
  ...
  const state=clone(inputState);state.guestVisits=visits;state.updatedAt=Date.now();
...
export function guestLoyalty(state,guestId){
  ...
  return {guest,visits,title:earned?.title??'Neu',next,complete:!next};
}

export function recordGuestService(inputState,sequence){
  const ensured=ensureGuestState(inputState),state=clone(ensured.state),guest=guestForSequence(sequence);
  const before=Math.max(0,Number(state.guestVisits[guest.id])||0),after=before+1;
  state.guestVisits[guest.id]=after;
  const milestone=GUEST_LOYALTY_MILESTONES.find(entry=>entry.visits===after)??null;
  const rewardCoins=milestone?.rewardCoins??0;
  if(rewardCoins)state.coins=Math.max(0,Number(state.coins)||0)+rewardCoins;
  state.updatedAt=Date.now();
  return {state,changed:true,guest:clone(guest),before,visits:after,milestone:milestone?clone(milestone):null,rewardCoins,loyalty:guestLoyalty(state,guest.id)};
}
