...
export function installPlaceMapUI(root){
  ...
  const ensureLauncher=()=>{if(root.dataset.view!=='place')return;const command=root.querySelector('.place-command');if(!command||command.querySelector('[data-action="place-map"]'))return;const button=document.createElement('button');button.className='place-map-launch';button.dataset.action='place-map';button.innerHTML='<span aria-hidden="true">⌖</span><b>Places</b><small>Weltkarte</small>';command.prepend(button);};
