...
export function placeMapModel(state){
  ...
  return PLACE_CHAPTERS.map(chapter=>{
    ...
    return {
      ...
      completed,
      total:chapter.upgrades.length,
      ratio:completed/chapter.upgrades.length,
      unlocked,
      active:chapter.id===active.id,
      complete:completed===chapter.upgrades.length,
    };
  });
}
...
function mapMarkup(state,selectedId){
  const places=placeMapModel(state);
  const selected=places.find(entry=>entry.id===selectedId&&entry.unlocked)||places.find(entry=>entry.active)||places[0];
...
export function installPlaceMapUI(root){
  let open=false,selectedId=null,overlay=null;
  const close=()=>{open=false;overlay?.remove();overlay=null;};
  const renderOverlay=()=>{
    if(!open)return;
    ...
    overlay?.remove();
    overlay=document.createElement('div');overlay.className='place-map-layer';overlay.innerHTML=mapMarkup(state,selectedId);document.body.append(overlay);
  };
  const ensureLauncher=()=>{
    if(root.dataset.view!=='place')return;
    const command=root.querySelector('.place-command');if(!command||command.querySelector('[data-action="place-map"]'))return;
    const button=document.createElement('button');button.className='place-map-launch';button.dataset.action='place-map';button.innerHTML='<span aria-hidden="true">⌖</span><b>Places</b><small>Weltkarte</small>';
    command.prepend(button);
  };
  root.addEventListener('click',event=>{
    const target=event.target instanceof Element?event.target:null;if(!target)return;
    if(target.closest('[data-action="place-map"]')){open=true;selectedId=activePlaceChapter(getState()).id;renderOverlay();}
  });
  document.addEventListener('click',event=>{
    const target=event.target instanceof Element?event.target:null;if(!target||!open)return;
    if(target.closest('[data-place-map-close]')){close();return;}
    const place=target.closest('[data-map-place]');if(place&&!place.disabled){selectedId=place.dataset.mapPlace;renderOverlay();}
  });
  document.addEventListener('keydown',event=>{if(event.key==='Escape'&&open)close();});
  new MutationObserver(ensureLauncher).observe(root,{childList:true,subtree:true});
  ensureLauncher();
