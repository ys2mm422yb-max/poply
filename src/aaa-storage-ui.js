import { getState, storeAt, restoreAt, expandStorage } from './aaa-session.js';
import { itemMarkup } from './aaa-view.js';
import { storageUpgradeCost, STORAGE_MAX_CAPACITY } from './aaa-inventory.js';
...
export function installStorageUI(root,ui){
  ...
  root.addEventListener('click',event=>{
    ...
    const store=target.closest('[data-storage-store]');if(store){
      ...
      else ui.message(result.reason==='storage-full'?'Lager voll – erweitern oder Item zurückholen.':'Dieses Item kann nicht eingelagert werden.','bad');
      return;
    }
    const restore=target.closest('[data-storage-restore]');if(restore){
      ...
      else ui.message(result.reason==='board-full'?'Werkbank voll – erst Platz schaffen.':'Item konnte nicht zurückgeholt werden.','bad');
      return;
    }
