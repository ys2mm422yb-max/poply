import { getState, storeAt, restoreAt, recycleStorageAt, expandStorage } from './aaa-session.js';
import { itemMarkup } from './aaa-view.js';
import { recycleCoinValue, storageUpgradeCost, STORAGE_MAX_CAPACITY } from './aaa-inventory.js';
...
const recycleIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m8.1 5.7 2-3.1 2 3.1M10.1 3v5.1M16.8 9.4l3.7.2-1.7 3.4M20 9.8l-4.5 2.6M14.2 18.3l-1.8 3.2-2-3.1M12.4 21l-.1-5.2M5.5 14.8l-3.6-.4 1.8-3.3M2.5 14.1l4.6-2.4" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';

export function installStorageUI(root,ui){
  let open=false,recycleMode=false,decorating=false;
  const stateNow=()=>getState();
  const storageSlot=(item,index)=>{
    if(!item)return '<div class="storage-slot empty" aria-hidden="true"><i></i></div>';
    if(recycleMode){const coins=recycleCoinValue(item);return `<button class="storage-slot occupied recycle-ready" data-storage-recycle="${index}" aria-label="${item.family} Stufe ${item.level} für ${coins} Coins recyceln">${itemMarkup(item)}<span>${item.level}</span><b class="storage-recycle-value"><i>${coinIcon}</i>${coins}</b></button>`;}
    return `<button class="storage-slot occupied" data-storage-restore="${index}" aria-label="${item.family} Stufe ${item.level} zurück auf die Werkbank">${itemMarkup(item)}<span>${item.level}</span></button>`;
  };
  ...
  const signature=state=>`${open?'1':'0'}:${recycleMode?'r':'n'}:${state.coins}:${state.storageCapacity}:${(state.storage||[]).map(item=>item?.id).join(',')}:${state.board.map(item=>item?.id||'').join(',')}`;
  const drawerMarkup=state=>{
    ...
    const recycle=storage.length?`<button class="storage-recycle-toggle ${recycleMode?'active':''}" data-storage-recycle-toggle aria-pressed="${recycleMode}"><i>${recycleIcon}</i><b>${recycleMode?'Abbrechen':'Recyceln'}</b></button>`:'';
    const sourceCopy=recycleMode?'<strong>Recycling aktiv</strong><small>Wähle bewusst ein Lager-Item · es wird gegen die angezeigten Coins entfernt.</small>':'<strong>Von der Werkbank</strong><small>Tippen = sicher einlagern · für Aufträge zurück aufs Board</small>';
    return `<section class="storage-drawer ${recycleMode?'recycle-mode':''}" aria-label="Lager"><header><div><small>WERKBANK-LAGER</small><strong>${storage.length}/${capacity} belegt</strong></div>${recycle}${upgrade}<button class="storage-close" data-storage-close aria-label="Lager schließen">×</button></header><div class="storage-slots">${slots}</div><div class="storage-source"><div>${sourceCopy}</div><div class="storage-board-items">${recycleMode?'<span class="storage-recycle-help">Recycling schafft garantiert einen Lagerplatz – auch wenn Werkbank und Lager voll sind.</span>':choices}</div></div></section>`;
  };
  const decorate=()=>{
  ...
  root.addEventListener('click',event=>{
    ...
    if(target.closest('[data-storage-toggle]')){open=!open;if(!open)recycleMode=false;decorate();return;}
    if(target.closest('[data-storage-close]')){open=false;recycleMode=false;decorate();return;}
    if(target.closest('[data-storage-recycle-toggle]')){recycleMode=!recycleMode;decorate();return;}
    const recycle=target.closest('[data-storage-recycle]');if(recycle){
      const result=recycleStorageAt(Number(recycle.dataset.storageRecycle));
      if(result.changed){recycleMode=false;ui.feedback('reward');ui.message(`Recycelt: +${result.coins} Coins · 1 Lagerplatz frei`);ui.render();decorate();}
      else ui.message('Dieses Lager-Item konnte nicht recycelt werden.','bad');
      return;
    }
    const store=target.closest('[data-storage-store]');if(store){
      ...
      else ui.message(result.reason==='storage-full'?'Lager voll – recyceln, erweitern oder Item zurückholen.':'Dieses Item kann nicht eingelagert werden.','bad');
      return;
    }
    const restore=target.closest('[data-storage-restore]');if(restore){
      ...
      else ui.message(result.reason==='board-full'?'Werkbank voll – im Lager Recyceln nutzen und danach ein Board-Item einlagern.':'Item konnte nicht zurückgeholt werden.','bad');
      return;
    }
  ...
  return {isOpen:()=>open,isRecycleMode:()=>recycleMode,refresh:decorate};
}
