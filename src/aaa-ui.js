...
export function createUI(root,toast){
  ...
  const render=()=>{
    ...
    if(lastFx?.index!=null&&view==='board'){const fx=lastFx;lastFx=null;requestAnimationFrame(()=>root.querySelector(`[data-index="${fx.index}"]`)?.classList.add(fx.type==='merge'?'fx-merge':'fx-spawn'));}
  };
  ...
  const spawn=index=>{const result=generateAt(index);if(!result.changed){message(result.reason==='board-full'?'Board voll – merge zuerst Items.':'Keine Energie.','bad');return;}lastFx={type:'spawn',index:result.spawnedIndex};buzz(7);message('Neues Item');render();};
