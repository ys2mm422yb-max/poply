...
export function installPlayerUI(root){
  ...
  const capacityMarkup=capacity=>capacity.complete
    ?`<div class="energy-capacity-roadmap complete" aria-label="Max-Energie vollständig ausgebaut mit ${capacity.currentMaxEnergy}"><span>MAX-ENERGIE</span><strong>${capacity.currentMaxEnergy}</strong><small>Voll ausgebaut</small></div>`
    :`<div class="energy-capacity-roadmap" aria-label="Max-Energie ${capacity.currentMaxEnergy}, nächste Erhöhung auf ${capacity.nextMaxEnergy} bei Level ${capacity.nextMilestoneLevel}"><span>MAX-ENERGIE</span><strong>${capacity.currentMaxEnergy} → ${capacity.nextMaxEnergy}</strong><small>+${capacity.gain} bei LV ${capacity.nextMilestoneLevel} · ${capacity.levelsAway} Level${capacity.levelsAway===1?'':'s'}</small></div>`;
  const nextLevelMarkup=(preview,capacity)=>`<div class="next-level-preview" aria-label="Nächstes Level ${preview.level}, ${preview.remainingXp} XP fehlen, ${preview.rewardCoins} Coins und volle Energie als Belohnung${preview.rewardMaxEnergyGain?`, plus ${preview.rewardMaxEnergyGain} Max-Energie`:''}"><span class="next-level-orb">LV ${preview.level}</span><div class="next-level-copy"><small>NÄCHSTES LEVEL</small><strong>${preview.remainingXp} XP fehlen</strong><div class="next-level-track"><i style="width:${Math.round(preview.ratio*100)}%"></i></div></div><div class="next-level-reward"><small>${preview.rewardMaxEnergyGain?`BELOHNUNG · MAX +${preview.rewardMaxEnergyGain}`:'BELOHNUNG'}</small><strong>+${preview.rewardCoins}</strong><span>Coins · Energie voll</span></div>${capacityMarkup(capacity)}</div>`;
  ...
  decorate();
  return {render:decorate,isMilestonesOpen:()=>milestonesOpen,completedMilestones:()=>completedMilestoneCount(getState()),completedPlaceBadges:()=>completedPlaceBadgeCount(getState()),playerTitle:()=>playerTitleProgress(getState()).current.label};
}
