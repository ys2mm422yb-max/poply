...
export function installPlayerUI(root){
  ...
  decorate();
  return {refresh:decorate,isMilestonesOpen:()=>milestonesOpen,completedMilestones:()=>completedMilestoneCount(getState()),completedPlaceBadges:()=>completedPlaceBadgeCount(getState()),playerTitle:()=>playerTitleProgress(getState()).current.label};
}
